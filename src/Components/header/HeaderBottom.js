...
const HeaderBottom = () => {
  ...
  return (
    ...
    <ul className="flex items-center gap-2 text-sm tracking-wide">
        <li onClick={()=>setSidebar(true)} className='headerHover flex gap-1 items-center'><MenuIcon/>All</li>
        ...
        <li className='headerHover hidden md:inline-flex'>Sells</li>
    </ul>    
        ...
        <div className='w-full h-screen text-black fixed top-0 left-0 bg-amazon_blue bg-opacity-50'>
            <div className='w-full h-full relative'>
                <motion.div ref={ref} initial={{x:-500,opacity:0}} animate={{x:0,opacity:1}} transition={{duration:.5}} className='w-[80%] md:w-[350px] h-full bg-white border border-black'>
                    ...
                    <SideNavContent title="Digital Content & Devices" one="Amazon Musics" two="kindle E-reader & Books" three="Amazon Appstore"/>
                    <SideNavContent title="Shop By Department" one="Electronics" two="Computers" three="Smart Home"/>
                    <SideNavContent title="Programs And Features" one="Gift Cards" two="Amazon Live" three="International Shopping"/>
